...
function normalizeSettings(settings) {
  ...
  normalized.textModel = String(normalized.textModel || DEFAULT_TEXT_MODEL).trim() || DEFAULT_TEXT_MODEL;
  normalized.imageModel = String(normalized.imageModel || DEFAULT_IMAGE_MODEL).trim() || DEFAULT_IMAGE_MODEL;
  return normalized;
}

function safeJsonParse(value, fallback) {
  if (!value) return normalizeSettings(fallback);
  try {
    return normalizeSettings({ ...fallback, ...JSON.parse(value) });
  } catch {
...
async function callGeminiImage({ apiKey, model, prompt }) {
  ...
  if (!inlineData?.data) {
    ...
    throw new Error(`画像データが返りませんでした。${reason ? `finishReason: ${reason}。` : ""}画像対応モデル（例: ${DEFAULT_IMAGE_MODEL}）とAPIキーの権限を確認してください。`);
  }
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4">
                ...
                {showAdvanced && <div className="mt-3 space-y-3">
                  <label className="block text-xs text-neutral-500">Text model</label>
              ...
              {concept.stamps.map((stamp, index) => {
                ...
                return <div key={stamp.id} className="overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900">
                  <div className="flex aspect-[370/320] items-center justify-center bg-neutral-950 p-2">
                    {img.src ? <img src={img.src} alt={stamp.label} className="h-full w-full object-contain" /> : <span className="px-2 text-center text-xs text-neutral-500">{img.status === "generating" ? "生成中..." : img.status === "error" ? "失敗" : "待機中"}</span>}
                  </div>
                  <div className="space-y-2 p-3">
                    ...
                    {img.error && <p className="line-clamp-3 break-words rounded-lg bg-red-950/40 p-2 text-[11px] leading-5 text-red-200">{img.error}</p>}
